...
PATCH NOTES (this revision):
  1. [CRITICAL BUG-FIX] Removed StandardScaler. Training was fitting
     LogisticRegression on standardized features (mean 0, std 1), but
     main.py's serving code (_linpred) applies the saved weights directly
     to RAW feature values — and the scaler's mean_/scale_ were never
     persisted anywhere, so there was no way to reproduce the transform at
     serving time even in principle. This made every live prediction
     effectively meaningless. Models are now trained directly on the same
     raw feature values main.py computes and feeds at inference time.
  2. [BUG-FIX] `scaler` was only defined inside the in-play training block
     but reused in the prematch block — if in-play training was skipped
     (e.g. not enough rows yet) but prematch had enough data, this raised
     a NameError and aborted training entirely.
  3. [SIDE-EFFECT OF #1] The train/test scaling leakage (scaler previously
     fit on train+test combined, before the time split) is gone along with
     the scaler itself.

  STILL OPEN (flagged, not silently patched): FEATURES / PRE_FEATURES below
  list ~60 engineered fields (total_shots_h, momentum_score, shot_accuracy_h,
  pm_rating_h/ELO fields, etc.) that the main.py you sent does NOT compute
  in extract_features()/extract_prematch_features() — those default to 0.0
  at serving time via feat.get(k, 0.0). If you have a harvester script that
...
def fit_lr_safe(X: np.ndarray, y: np.ndarray) -> Optional[LogisticRegression]:
    if len(np.unique(y)) < 2:
        return None
    # PATCH: no scaler upstream anymore — trained directly on raw features,
    # matching what main.py's _linpred() will multiply weights against at
    # serving time. liblinear handles unscaled features fine for this size.
...
def train_models(
    ...
    try:
        ...
        if not df_ip.empty and len(df_ip) >= min_rows:
            ...
            # PATCH: train directly on raw features — no StandardScaler.
            # main.py's serving code applies saved weights to raw feature
            # values, so the model must be fit on the same raw scale or the
            # weights are meaningless at inference time.
        ...
        if not df_pre.empty and len(df_pre) >= min_rows:
            ...
            # PATCH: same as above — raw features, no scaler.
            ...
            if ok_h and ok_a and (p_h is not None) and (p_a is not None):
                # Safe probability normalization for prematch
                p_h_safe = np.clip(p_h, EPS, 1 - EPS)
                p_a_safe = np.clip(p_a, EPS, 1 - EPS)
        ...
        logger.info("Data stats: %s", summary["data_stats"])
...
def log_learning_insights(conn, df_ip: pd.DataFrame, df_pre: pd.DataFrame):
    ...
    if insights:
        ...
        # Save to database for the learning system
        try:
            _exec(conn, """
