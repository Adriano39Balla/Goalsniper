...
def load_data(db_path: str, min_minute: int = 15):
    ...
    # join last snapshot per match_id (so we don't overweight late spam)
    q = """
    WITH latest AS (
      SELECT match_id, MAX(created_ts) AS ts
      FROM tip_snapshots GROUP BY match_id
    )
    SELECT l.match_id, s.created_ts, s.payload, r.final_goals_h, r.final_goals_a, r.btts_yes
    FROM latest l
...
def fit_lr(X, y):
    # class_weight balances class imbalance a bit; liblinear works well on small sets
...
def main():
    ...
    df_o25, df_btts = load_data(args.db, args.min-minute if hasattr(args, "min-minute") else args.min_minute)
    ...
    # O2.5
    Xo = df_o25[FEATURES].values; yo = df_o25["label_o25"].values
    ...
    # BTTS Yes
    Xb = df_btts[FEATURES].values; yb = df_btts["label_btts"].values
