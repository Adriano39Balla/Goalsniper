...
# Model‑only mode knobs (no odds / no extra gates)
...
INPLAY_STATUSES  = {"1H","HT","2H","ET","BT","P"}  # allow all in‑play codes
...
def fetch_live_matches() -> List[Dict[str, Any]]:
    ...
    for m in matches:
        ...
        if elapsed is None or elapsed > 120:  # keep it generous; no “minute gate”
            continue
...
# ──────────────────────────────────────────────────────────────────────────────
# Model scoring (no odds / no gates)
...
def _apply_simple_threshold(
    ...
    # keep only those passing MIN_PROB, then top‑k by prob
    keep = [(mkt, sugg, p, head) for (mkt, sugg, p, head) in suggestions if p >= min_prob]
    keep.sort(key=lambda x: x[2], reverse=True)
    return keep[:max(1, int(top_k))]
    ...
    with db_conn() as conn:
        for m in matches:
            try:
                ...
                filtered = _apply_simple_threshold(raw, min_prob=MIN_PROB, top_k=TOP_K_PER_MATCH)
...
def match_of_the_day_job():
    """Pick the highest-confidence tip saved in the last 24h and send a trophy card."""
    try:
        now = int(time.time()); day_ago = now - 24*3600
        with db_conn() as conn:
            row = conn.execute(
...
# optional manual trigger for MOTD
@app.route("/motd")
def motd_route():
    _require_api_key()
...
if __name__ == "__main__":
    ...
    # Match of the Day at 09:05 local time
