...
# ADDED: Thread safety for critical sections
...
def _metric_inc(name: str, label: Optional[str] = None, n: int = 1) -> None:
    try:
        with _METRICS_LOCK:  # FIXED: Added thread safety
            if label is None:
            ...
            # FIXED: Auto-clean large collections to prevent memory leaks
            if name == "job_duration_seconds" and isinstance(METRICS[name], dict):
                for job_name, durations in METRICS[name].items():
                    if len(durations) > 100:
                        METRICS["job_duration_seconds"][job_name] = durations[-50:]
    except Exception:
        pass
...
# FIXED: Thread-safe database pool initialization
def _init_pool():
    global POOL
    with _POOL_LOCK:  # FIXED: Added thread safety
        if POOL:
            return
...
class AdvancedEnsemblePredictor:
    ...
    # FIXED: Added proper validation for ensemble predictions
    def predict_ensemble(self, features: Dict[str, float], market: str, minute: int) -> Tuple[float, float]:
        ...
        for model_type in self.model_types:
            try:
                ...
                # FIXED: Validate both probability and confidence values
                if (prob is not None and 
                    ...
                    log.warning(f"[ENSEMBLE] Invalid prediction from {model_type}: prob={prob}, conf={confidence}")
            except Exception as e:
        ...
        # FIXED: Final validation of output values
...
class CacheManager:
    ...
    def cleanup_all_caches(self):
        ...
        self._cleanup_neg_cache(NEG_CACHE, self.max_neg_cache_size)  # FIXED: Added NEG_CACHE cleanup
        ...
        self._cleanup_old_entries_neg(NEG_CACHE, NEG_TTL_SEC)  # FIXED: Added NEG_CACHE aging
    ...
    # FIXED: Added specific cleanup for NEG_CACHE structure
    def _cleanup_neg_cache(self, cache: dict, max_size: int):
        if len(cache) > max_size:
            keys_to_remove = list(cache.keys())[:len(cache) - max_size]
            for key in keys_to_remove:
                cache.pop(key, None)
    ...
    # FIXED: Added specific aging for NEG_CACHE
    def _cleanup_old_entries_neg(self, cache: dict, max_age_seconds: int):
        current_time = time.time()
        keys_to_remove = []
        for key, value in cache.items():
            if isinstance(value, tuple) and len(value) > 0:
...
def check_prediction_quality():
    ...
    test1 = {"minute": 25.0,"goals_h": 1.0,"goals_a": 1.0,"xg_h": 1.2,"xg_a": 0.8,"sot_h": 5.0,"sot_a": 4.0,"pos_h": 55.0,"pos_a": 45.0,"pressure_home": 60.0,"pressure_away": 50.0,"goals_sum": 2.0,"xg_sum": 2.0,"goals_last_15": 1.0}
