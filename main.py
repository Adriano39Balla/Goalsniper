...
# All runtime behavior is env-driven. Prefer tweaking .env over code changes.
...
# IMPORTANT: no baked defaults here; drive from env only (falls back to empty → function fallback applies)
MARKET_CUTOFFS_RAW = os.getenv("MARKET_CUTOFFS", "")
TIP_MAX_MINUTE_ENV = os.getenv("TIP_MAX_MINUTE", "")

if not ADMIN_API_KEY:
    log.warning("ADMIN_API_KEY not set — admin endpoints disabled.")
if not WEBHOOK_SECRET:
    log.warning("TELEGRAM_WEBHOOK_SECRET not set — webhook route would be unsafe if exposed.")
...
class PooledConn:
    ...
    def execute(self, sql: str, params: tuple|list=()):
        ...
        try:
            ...
            log.error("DB execute failed: %s\nSQL: %s\nParams: %s", e, sql, params); raise
    def executemany(self, sql: str, seq_of_params: list[tuple] | list[list]):
        if ShutdownManager.is_shutdown_requested():
            raise Exception("Database operation refused - shutdown in progress")
        try:
            self.cur.executemany(sql, seq_of_params or []); return self.cur
        except Exception as e:
            _metric_inc("db_errors_total", n=1)
            log.error("DB executemany failed: %s\nSQL: %s\nBatch size: %s", e, sql, len(seq_of_params or [])); raise
    def fetchone_safe(self):
        try:
            ...
            log.warning("[DB] fetchone_safe error: %s", e); return None
    def fetchall_safe(self):
        try:
            rows = self.cur.fetchall(); return rows if rows else []
        except Exception as e:
            log.warning("[DB] fetchall_safe error: %s", e); return []
...
def _db_ping() -> bool:
    if ShutdownManager.is_shutdown_requested(): return False
    try:
        with db_conn() as c:
            c.execute("SELECT 1"); return True
    except Exception:
        log.warning("[DB] ping failed, re-initializing pool")
        try:
            _init_pool()
            with db_conn() as c2:
                c2.execute("SELECT 1"); return True
        except Exception as e:
            _metric_inc("db_errors_total", n=1)
            log.error("[DB] reinit failed: %s", e); return False
...
class _KVCache:
    ...
    def set(self,k,v):
        if _redis:
            try:
                _redis.setex(f"gs:{k}", self.ttl, v if v is not None else ""); return
            except Exception:
                pass
        self.data[k]=(time.time(),v)
    def invalidate(self,k=None):
        if _redis and k:
            try: _redis.delete(f"gs:{k}"); return
...
def _api_get(url: str, params: dict, timeout: int = 15):
    ...
    if API_CB["opened_until"] > now:
        log.warning("[CB] Circuit open, rejecting request to %s", url); return None
    if API_CB["failures"] > 0 and now - API_CB.get("last_success", 0) > API_CB_COOLDOWN_SEC:
        ...
        API_CB["failures"] = 0; API_CB["opened_until"] = 0
    lbl = "unknown"
    try:
...
def extract_basic_features(m: dict) -> Dict[str,float]:
    ...
    # Why: improve signal when API xG missing
    if xg_h == 0 and xg_a == 0:
        if (sot_h + sot_a) > 0:
            xg_h = max(xg_h, sot_h * 0.30)
            xg_a = max(xg_a, sot_a * 0.30)
        elif (sh_total_h + sh_total_a) > 0:
            xg_h = sh_total_h * 0.10
            xg_a = sh_total_a * 0.10
            log.info(f"[XG_ESTIMATE] Using SHOTS-based xG: {xg_h:.2f}-{xg_a:.2f}")
    if (xg_h + xg_a) > 0:
        log.info(f"[XG_REAL_or_EST] xG now: {xg_h:.2f}-{xg_a:.2f}")
    else:
...
# ───────── Models & predictors (unchanged APIs) ─────────
class AdvancedEnsemblePredictor:
    ...
    def _predict_single_model(self, features: Dict[str, float], market: str, minute: int, model_type: str) -> Tuple[Optional[float], float]:
        if model_type == 'logistic': return self._logistic_predict(features, market), 0.8
        elif model_type == 'xgboost': return self._xgboost_predict(features, market), 0.85
        elif model_type == 'neural': return self._neural_network_predict(features, market), 0.75
        elif model_type == 'bayesian': return self._bayesian_predict(features, market, minute), 0.9
        elif model_type == 'momentum': return self._momentum_based_predict(features, market, minute), 0.7
        return None, 0.0
    ...
    def _neural_network_predict(self, features: Dict[str, float], market: str) -> Optional[float]:
        try:
            ...
            if base_prob <= 0.0 or base_prob >= 1.0: return base_prob
    ...
    def _calculate_nn_correction(self, features: Dict[str, float], market: str) -> float:
        ...
        for key, value in features.items():
            if "xg" in key: non_linear_features.append(value ** 1.5)
            elif "pressure" in key: non_linear_features.append(np.tanh(value / 50))
            else: non_linear_features.append(value)
        if market == "BTTS": return sum(non_linear_features) * 0.01
        else: return sum(non_linear_features) * 0.005
    ...
    def _calculate_momentum_factor(self, features: Dict[str, float], minute: int) -> float:
        ...
        momentum = 0.0
        goals_last_15 = features.get("goals_last_15", 0); momentum += goals_last_15 * 0.2
        shots_last_15 = features.get("shots_last_15", 0); momentum += shots_last_15 * 0.05
        recent_xg_impact = features.get("recent_xg_impact", 0); momentum += recent_xg_impact * 0.1
        return momentum
    def _calculate_pressure_factor(self, features: Dict[str, float]) -> float:
        ...
        if abs(score_advantage) <= 1: return abs(pressure_diff) * 0.01
        else: return pressure_diff * 0.005
    ...
    def _get_recent_performance(self, model_type: str, market: str) -> float: return 0.9
    def _calculate_time_weight(self, minute: int, model_type: str) -> float:
        if model_type in ['bayesian', 'momentum']: return min(1.0, minute / 60.0)
        else: return 1.0
...
class AdvancedFeatureEngineer:
    def __init__(self):
        self.feature_cache = {}; self.temporal_patterns = {}
...
class MarketSpecificPredictor:
    ...
    def _predict_ou_advanced(self, features: Dict[str, float], minute: int, line: Optional[float] = None, market_key: Optional[str] = None) -> Tuple[float, float]:
        ...
        if base_prob <= 0.0: return 0.0, 0.0
...
# ───────── Adaptive learning (unchanged API) ─────────
class AdaptiveLearningSystem:
    ...
    def analyze_performance_trends(self) -> Dict[str, Any]:
        ...
        for key, history in self.performance_history.items():
            ...
            correct = sum(1 for h in history if h['outcome'] == 1); total = len(history)
...
def predict_from_model(mdl: Dict[str, Any], features: Dict[str, float]) -> float:
    ...
    try:
        ...
        if cal:
            method = cal.get("method", "sigmoid").lower(); a = float(cal.get("a", 1.0)); b = float(cal.get("b", 0.0))
            if method == "sigmoid":
...
def fetch_odds(fid: int, prob_hints: Optional[dict[str, float]] = None, require_live: bool = True) -> dict:
    ...
    if (js.get("response") or []):
        ...
        NEG_CACHE[k] = (now, True); ODDS_CACHE[fid] = (time.time(), {}); return {}
    ...
    def _aggregate_price(vals: list[tuple[float, str]], prob_hint: Optional[float]) -> tuple[Optional[float], Optional[str]]:
        ...
        if ODDS_AGGREGATION == "best":
            best = max(filtered, key=lambda t: t[0]); return float(best[0]), str(best[1])
        target = med2; pick = min(filtered, key=lambda t: abs(t[0] - target))
    ...
    if not out: NEG_CACHE[k] = (now, True)
    else: log.info(f"[ODDS] Successfully parsed {len(out)} markets for fixture {fid}")
    return out
...
# ───────── Confidence gate (env-driven) ─────────
CONF_GATE_EARLY = float(os.getenv("CONF_GATE_EARLY", "0.50"))
CONF_GATE_MID   = float(os.getenv("CONF_GATE_MID",   "0.40"))
CONF_GATE_LATE  = float(os.getenv("CONF_GATE_LATE",  "0.25"))

def confidence_gate_ok(market: str, conf: float | None, minute: int, feat: dict) -> bool:
    if conf is None: return False
    try: m = int(minute)
    except Exception: m = 0
    if m < 20: thr = CONF_GATE_EARLY
    elif m <= 60: thr = CONF_GATE_MID
    else: thr = CONF_GATE_LATE
    # Why: relax when data is thin
    if (float(feat.get("xg_sum", 0)) <= 0.0) and (float(feat.get("sot_sum", 0)) <= 0.0):
        thr = max(0.05, thr - 0.10)
    return float(conf) >= thr

# ───────── Cache manager ─────────
class CacheManager:
    def __init__(self):
        self.max_stats_cache_size = int(os.getenv("MAX_STATS_CACHE", "1000"))
        self.max_events_cache_size = int(os.getenv("MAX_EVENTS_CACHE", "1000"))
        self.max_odds_cache_size = int(os.getenv("MAX_ODDS_CACHE", "500"))
        self.max_neg_cache_size = int(os.getenv("MAX_NEG_CACHE", "1000"))
    def cleanup_all_caches(self):
        ...
        self._cleanup_old_entries(STATS_CACHE, int(os.getenv("TTL_STATS_SEC","300")))
        self._cleanup_old_entries(EVENTS_CACHE, int(os.getenv("TTL_EVENTS_SEC","300")))
        self._cleanup_old_entries(ODDS_CACHE, int(os.getenv("TTL_ODDS_SEC","600")))
    ...
    def _cleanup_cache(self, cache: dict, max_size: int):
        if len(cache) > max_size:
            ...
            for key in keys_to_remove: cache.pop(key, None)
...
class ConfigValidator:
    ...
    def _check_required_vars(self):
        ...
        if missing: raise SystemExit(f"Missing required environment variables: {', '.join(missing)}")
    def _check_optional_vars(self):
        for var, message in self.optional_env_vars.items():
            if not os.getenv(var): log.warning(f"{message} ({var} not set)")
    ...
    def _check_api_connectivity(self):
        try:
            ...
            if test_resp is not None: log.info("[CONFIG] API-Football connectivity: OK")
            else: log.warning("[CONFIG] API-Football test request failed")
        except Exception as e:
...
def stats_coverage_ok(feat: Dict[str, float], minute: int) -> bool:
    try:
        ...
        log.error("[stats_coverage_ok] error: %s", e); return False
...
def is_feed_stale(fid: int, m: dict, minute: int) -> bool:
    ...
    try:
        ...
        if events:
            ...
            if minute - latest_event_minute > 10 and minute > 30: return True
        cache_time = STATS_CACHE.get(fid, (0, []))[0]
        if time.time() - cache_time > STALE_STATS_MAX_SEC: return True
        return False
    except Exception as e:
        log.error("[is_feed_stale] error: %s", e); return False
...
def save_snapshot_from_match(m: dict, feat: Dict[str, float]) -> None:
    try:
        ...
        log.error("[save_snapshot_from_match] %s", e)

def _league_name(m: dict) -> Tuple[int, str]:
    try:
        league = m.get("league", {}) or {}
        league_id = int(league.get("id", 0))
        country = league.get("country", ""); name = league.get("name", "")
...
def _get_market_threshold(market: str) -> float:
    try:
        ...
        log.error("[_get_market_threshold] %s", e); return CONF_THRESHOLD
    ...
    try:
        ...
        if suggestion.startswith("Under"):
            try:
                ...
                if line and goals_sum > line: return False
            except: pass
        if suggestion == "BTTS: No":
            goals_h = feat.get("goals_h", 0); goals_a = feat.get("goals_a", 0)
            if goals_h > 0 and goals_a > 0: return False
        return True
    except Exception:
        return False
...
def _inplay_1x2_sanity_ok(suggestion: str, feat: Dict[str, float], odds: Optional[float]) -> bool:
    try:
        minute = int(feat.get("minute", 0)); gd = int(feat.get("goals_h", 0) - feat.get("goals_a", 0))
        if suggestion not in ("Home Win", "Away Win"): return True
        if minute >= 60 and abs(gd) >= 2 and odds is not None and odds > 1.50: return False
        return True
    except Exception:
        return False
...
class BayesianUpdater:
    def __init__(self): self.prior_strength = 0.3
...
class GameStateAnalyzer:
    ...
    def analyze_game_state(self, feat: Dict[str, float]) -> Dict[str, float]:
        ...
        minute = int(feat.get("minute", 0)); total_goals = float(feat.get("goals_sum", 0))
...
class SmartOddsAnalyzer:
    ...
    def _market_odds_quality(self, sides: dict, prob_hint: float | None) -> float:
        ...
        for data in (sides or {}).values():
            o = (data or {}).get("odds")
            if o and o > 0: total_implied += 1.0 / float(o); valid_odds.append(float(o))
    ...
    def _resolve_hint_for_specific(self, hints: dict[str, float], market_key: str) -> float | None:
        ...
        if market_key == "BTTS": return hints.get("BTTS: Yes")
        if market_key == "1X2":
            hw = hints.get("Home Win") or 0.0; aw = hints.get("Away Win") or 0.0
            return max(hw, aw) or None
        if market_key.startswith("OU_"):
            try:
                line_txt = market_key.split("_", 1)[1]; return hints.get(f"Over {line_txt} Goals")
            except Exception:
                return None
        return None
...
def enhanced_production_scan() -> Tuple[int, int]:
    try:
        if not _db_ping():
            log.error("[ENHANCED_PROD] Database unavailable"); return (0, 0)
        try:
            matches = fetch_live_matches()
        except Exception as e:
            log.error("[ENHANCED_PROD] Failed to fetch live matches: %s", e); return (0, 0)
        live_seen = len(matches)
        if live_seen == 0:
            log.info("[ENHANCED_PROD] no live matches"); return 0, 0
            ...
            for m in matches:
                try:
                    ...
                    try:
                        ...
                        if btts_prob > 0 and confidence_gate_ok("BTTS", btts_conf, minute, feat):
                    ...
                    for line in OU_LINES:
                        ...
                        try:
                            ...
                            if ou_prob <= 0 or not confidence_gate_ok("OU", ou_conf, minute, feat):
                                _dbg("ou_low_conf", line=_fmt_line(line), conf=round(ou_conf,3), minute=minute); continue
                    ...
                    try:
                        ...
                        if ph > 0 and pa > 0 and confidence_gate_ok("1X2", c1, minute, feat):
                    ...
                    for mk, sug, prob, conf in candidates:
                        ...
                        # Why: allow pass when we explicitly allow tips without odds and there is no odds_map
                        if oq < oa.odds_quality_threshold and not (ALLOW_TIPS_WITHOUT_ODDS and not odds_map):
                        ...
                        if not pass_odds:
                            if ALLOW_TIPS_WITHOUT_ODDS:
                                _dbg("price_gate_soft_fail_but_allowed", market=mk, sug=sug, reason="no live odds", allow_without_odds=True)
                                odds, book = None, None
                            else:
                                _dbg("price_gate_block", market=mk, sug=sug, have_odds=bool(odds_map), min=_min_odds_for_market(mk))
                                continue
                        else:
                            if odds is None:
                    ...
                    for idx, (market_txt, suggestion, prob, odds, book, ev_pct, _rank, conf) in enumerate(ranked):
                        ...
                        try:
                            ...
                            sent = send_telegram(_format_tip_message(
                                ...
                                float(prob_pct), {}, odds, book, ev_pct
                            ))
                            if sent:
                                with db_conn() as c3:
...
# ───────── Results/backfill/digest/retry (unchanged) ─────────
...
def backfill_results_for_open_matches(max_rows: int = 200) -> int:
    now_ts = int(time.time()); cutoff = now_ts - BACKFILL_DAYS * 24 * 3600
    updated = 0
    with db_conn() as c:
        rows = c.execute("""
            WITH last AS (
...
def auto_train_job():
    if not TRAIN_ENABLE: return send_telegram("🤖 Training skipped: TRAIN_ENABLE=0")
...
def retry_unsent_tips(minutes: int = 30, limit: int = 200) -> int:
    cutoff = int(time.time()) - minutes*60; retried = 0
    with db_conn() as c:
        rows = c.execute(
...
def shutdown_handler(signum=None, frame=None, *, from_atexit: bool = False):
    ...
    _SHUTDOWN_RAN = True
    try: log.info("Received shutdown (%s), cleaning up...", "atexit" if from_atexit else ("signal" if signum else "manual"))
    except Exception: pass
...
def _start_scheduler_once():
    ...
    try:
        ...
        send_telegram("🚀 goalsniper AI mode (in-play) started.")
...
def debug_live_match(fid: int):
    ...
    if not match: return jsonify({"error": "Match not found", "fixture_id": fid})
...
def http_debug_scan():
    _require_admin()
    # Keep minimal to avoid log spam; use DEBUG_SELECTION_LOG to govern verbosity
    s,l = production_scan()
    return jsonify({"ok": True, "saved": s, "live_seen": l})
...
def telegram_webhook(secret: str):
    ...
    try:
        ...
        if msg.startswith("/start"): send_telegram("👋 goalsniper bot (IN-PLAY AI ENHANCED) is online.")
...
def _on_boot():
    try:
        ...
        set_setting("boot_version", "2.1.0-env")
        _start_scheduler_once()
        log.info("[BOOT] goalsniper AI system started successfully")
    except Exception as e:
