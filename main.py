# file: main.py
"""
goalsniper — FULL AI mode (in-play + prematch) with odds + EV gate + Tipico league filter.
...
- League allow/deny lists for Tipico-compatible coverage.
"""
...
MOTD_CONF_MIN           = float(os.getenv("MOTD_CONF_MIN", "75"))
MOTD_MIN_EV_BPS         = int(os.getenv("MOTD_MIN_EV_BPS", "900"))  # +9% EV min
try:
...
EDGE_MIN_BPS  = int(os.getenv("EDGE_MIN_BPS", "700"))  # +7.00% min edge
...
def _ev(prob: float, odds: float) -> float:
    """Expected value as decimal (0.05 = +5%)."""
            ...
            for mkt in (bk.get("bets") or []):
                ...
                if mname == "BTTS":
                    ...
                    for v in vals:
                        ...
                        if "yes" in lbl:
                            d["Yes"] = {"odds": float(v.get("odd") or 0), "book": book_name}
                        if "no" in lbl:
                            d["No"] = {"odds": float(v.get("odd") or 0), "book": book_name}
                    if d:
                        out["BTTS"] = d
                ...
                # 1X2
                    ...
                    for v in vals:
                        lbl = (v.get("value") or "").strip().lower()
                        if lbl in ("home", "1"):
                            d["Home"] = {"odds": float(v.get("odd") or 0), "book": book_name}
                        if lbl in ("away", "2"):
                            d["Away"] = {"odds": float(v.get("odd") or 0), "book": book_name}
                    if d:
                        out["1X2"] = d
                    ...
                    for v in vals:
                        ...
                        if "over" in lbl or "under" in lbl:
                            try:
                                ...
                                by_line.setdefault(key, {}).update({
                                    side: {"odds": float(v.get("odd") or 0), "book": book_name}
                                })
                            except Exception:
                                pass
...
def _fixture_by_id(match_id: int) -> Optional[dict]:
    js = _api_get(FOOTBALL_API_URL, {"id": match_id}) or {}
...
def daily_accuracy_digest() -> Optional[str]:
    ...
    with db_conn() as c:
        rows = c.execute(
            ...
            WHERE t.created_ts >= %s AND t.created_ts < %s
              ...
              AND t.sent_ok = 1
            """,
            (int(y0.timestamp()), int(y1.timestamp())),
        ).fetchall()
    ...
    if graded == 0:
        ...
        lines = [
            "📊 <b>Daily Digest</b> (yesterday, Berlin time)",
...
# ───────── Threshold helpers & message formatting ─────────
...
def _format_tip_message(home, away, league, minute, score, suggestion, prob_pct, feat, odds=None, book=None, ev_pct=None):
    stat = ""
    if any([
        feat.get("xg_h", 0), feat.get("xg_a", 0),
        feat.get("sot_h", 0), feat.get("sot_a", 0),
        feat.get("cor_h", 0), feat.get("cor_a", 0),
        feat.get("pos_h", 0), feat.get("pos_a", 0),
...
# ───────── In-play scan (with odds/EV gate) ─────────
        ...
        for m in matches:
            try:
                ...
                if mh and md and ma:
                    ...
                    pd = _score_prob(feat, md)  # not emitted, but included for normalization
...
# ───────── Prematch features + scan (save-only) ─────────
...
def extract_prematch_features(fx: dict) -> Dict[str, float]:
    ...
    def _rate(games: List[dict]) -> Tuple[float, float, float]:
    ...
    return {
        ...
        # keep live feature keys (zeros) for model compatibility
...
def prematch_scan_save() -> int:
    ...
    for fx in fixtures:
        ...
        # odds/EV, save-only (no auto-send here)
        candidates.sort(key=lambda x: x[2], reverse=True)
        base_now = int(time.time())
        per_match = 0
...
# ───────── MOTD (prematch) with EV & thresholds ─────────
MOTD_MIN_EV_BPS = int(os.getenv("MOTD_MIN_EV_BPS", "0"))  # 0 disables EV gate for MOTD
...
def send_match_of_the_day() -> bool:
    if not MOTD_PREDICT:
        return send_telegram("🏅 Match of the Day: prematch models disabled (MOTD_PREDICT=0).")

    fixtures = _collect_todays_prematch_fixtures()
    if not fixtures:
        return send_telegram("🏅 Match of the Day: no eligible fixtures today.")

    # Optional league allow-list specific to MOTD
    if MOTD_LEAGUE_IDS:
    ...
    for fx in fixtures:
        ...
        # take the strongest candidate by prob, then check MOTD conf + EV
        candidates.sort(key=lambda x: x[2], reverse=True)
...
def http_tips_by_match(match_id: int):
    with db_conn() as c:
        rows = c.execute(
            "SELECT league,home,away,market,suggestion,confidence,confidence_raw,score_at_tip,minute,created_ts,odds,book,ev_pct,sent_ok "
    ...
    for r in rows:
        out.append({
            ...
            "score_at_tip": r[7], "minute": int(r[8]), "created_ts": int(r[9]),
            "odds": (float(r[10]) if r[10] is not None else None), "book": r[11],
            "ev_pct": (float(r[12]) if r[12] is not None else None),
            "sent_ok": int(r[13]),
        })
