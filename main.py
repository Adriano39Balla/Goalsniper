    ...
    for i in range(0, len(ids), MAX_IDS_PER_REQ):
        ...
        for _ in range(2):
            ...
            if not missing:
                resp_ok = True
                break
            time.sleep(0.35)  # small backoff before retry

        # Per-ID fallback for whatever is still missing
        if not resp_ok:
            for fid in missing:
                js1 = _api_get(FOOTBALL_API_URL, {"id": fid, "timezone": "UTC"})
                r1 = (js1.get("response") if isinstance(js1, dict) else []) or []
...
# ── Routes ────────────────────────────────────────────────────────────────────
@app.route("/stats/overall", methods=["GET"])
def stats_overall():
    _require_api_key()
    with db_conn() as conn:
    ...
    return jsonify({
        "totals": {
            "tip_snapshots": int(total_snaps),
            "match_results": int(total_results),
            "fully_labeled_matches": int(fully_labeled),
